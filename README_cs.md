...
Komunikace probíhá přímo přes lokální síť (LAN/WiFi) pomocí HTTP requestů na Pocket Wi-Fi dongle, bez závislosti na cloudu a s bleskovou odezvou.
...
Klikněte na tlačítko **Přidat** (Configure). Integrace automaticky předvyplní zjištěnou IP adresu, vy pouze zadáte přístupové heslo k API (často shodné se sériovým číslem donglu).
...
> [!TIP]
> Senzory `_total` mají nastavenou třídu `TOTAL_INCREASING`, což je vyžadováno pro správné dlouhodobé statistiky.
